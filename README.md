...
### Zebrafish (`simulations/zebrafish/`)

**Status**: MuJoCo embodied larval zebrafish with a source-annotated PAULA
connectome scaffold.

- 5-7 dpf larval body as a MuJoCo segmented swimmer with 3D root motion,
  yaw/pitch tail joints, and per-quadrant tail actuators.
- Bout-and-coast tail-wave locomotion with optic-flow, odor, lateral-line,
  thermal, wall, and startle sensory channels.
- PAULA-backed reduced tectum/pretectum/hindbrain/reticulospinal/spinal CPG
  circuit, ready to swap for a denser public export when one is available.
- Research corpus under `simulations/zebrafish/research/` tracks public
  connectome, calcium/voltage imaging, behavior, anatomy, and body resources.
...
**Live browser demos & virtual labs:**
- **C. elegans**: [https://jimmy.arteriali.st](https://jimmy.arteriali.st) — WebSocket + canvas viewer for the C. elegans simulation stack, served from the sibling [live-c-elegans](https://github.com/arterialist/live-c-elegans) package.
- **Larval Zebrafish**: [live-zebrafish](https://github.com/arterialist/live-zebrafish) — WebSocket canvas demo and React-based virtual lab dashboard for the larval zebrafish simulation stack.
...
# Run the MuJoCo larval zebrafish simulation
uv run python scripts/run_zebrafish.py --steps 1000
```
