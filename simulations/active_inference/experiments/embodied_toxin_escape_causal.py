"""Short-horizon embodied toxin-survival acceptance test.

One diagonal physical toxin source is placed on the normal MuJoCo course and
all food/other toxins are moved out of range.  The intact PAULA circuit must
remain contact-free while the ``w_tox=0`` control retains identical toxin
sensor currents and motor physics but enters the hazard.  This is deliberately
a local toxin-avoidance claim, not a head-on-hazard or lifetime-survival claim.
"""
...
FAR = [100.0, 100.0]
WORLD_SPECS = {
    "near_left": {"source": [-0.8, 0.30], "label": "near left hazard", "completion_steps": 160},
    "near_right": {"source": [-0.8, -0.30], "label": "near right hazard", "completion_steps": 160},
    "mid_left": {"source": [-1.0, 0.35], "label": "mid left hazard", "completion_steps": 180},
    "mid_right": {"source": [-1.0, -0.35], "label": "mid right hazard", "completion_steps": 180},
    "deep_left": {"source": [-1.3, 0.30], "label": "deep left hazard", "completion_steps": 200},
    "deep_right": {"source": [-1.3, -0.30], "label": "deep right hazard", "completion_steps": 200},
    "far_left": {"source": [-1.6, 0.25], "label": "far left hazard", "completion_steps": 240},
    "far_right": {"source": [-1.6, -0.25], "label": "far right hazard", "completion_steps": 240},
    "cross_left": {"source": [-1.9, 0.20], "label": "cross-body left hazard", "completion_steps": 280},
    "cross_right": {"source": [-1.9, -0.20], "label": "cross-body right hazard", "completion_steps": 280},
}
WORLD_NAMES = tuple(WORLD_SPECS)
CASES = {
    "toxin_avoidance": {"build": {}},
    "toxin_turn_ablation": {"build": {"w_tox": 0.0}},
}
...
def _install_hazard_world(agent, world: str) -> None:
...
def run_case(seed: int, name: str, case: dict, steps: int, substeps: int, version: str, world: str,
    ...
    _install_hazard_world(agent, world)
    agent.birth()
...
def _summary(record: dict) -> dict:
    ...
    toxin_left = sum(row["toxin_sensor_spikes"]["left"] for row in trace)
    toxin_right = sum(row["toxin_sensor_spikes"]["right"] for row in trace)
    tl = sum(row["turn_spikes"]["TL"] for row in trace)
    tr = sum(row["turn_spikes"]["TR"] for row in trace)
    return {
        "world": record["world"],
        "toxin_hits": record["toxin_hits"],
        "toxin_sensor_spikes": toxin_left + toxin_right,
...
def _accept(summaries: dict[str, dict[str, dict[str, dict]]], version: str) -> list[str]:
    ...
    for world, by_seed in summaries.items():
        for seed, rows in by_seed.items():
            full, ablated = rows["toxin_avoidance"], rows["toxin_turn_ablation"]
            ...
            for label, row in rows.items():
                ...
                if not row["fixture_no_respawn"] or row["fixture_food_count"] != 7 or row["fixture_toxin_count"] != 6:
                    failures.append(f"{prefix} {label}: toxin fixture was changed or permits respawn")
            if full["toxin_sensor_spikes"] < 100 or ablated["toxin_sensor_spikes"] < 100:
                failures.append(f"{prefix}: physical toxin did not drive both test conditions")
            if full["toxin_hits"] != 0:
                failures.append(f"{prefix}: intact toxin circuit entered the hazard")
            if min(full["max_abs_actuator_control"], ablated["max_abs_actuator_control"]) <= 0.0:
                failures.append(f"{prefix}: toxin test did not reach the normal MuJoCo actuator path")
    return failures


def _inconclusive(summaries: dict[str, dict[str, dict[str, dict]]]) -> list[str]:
    """Controls that never encounter the hazard cannot establish causality."""
    return [
        f"{world} seed {seed}: toxin-turn ablation never entered the contact zone "
        "(fixture is inconclusive for this replicate)"
        for world, by_seed in summaries.items()
        for seed, rows in by_seed.items()
        if rows["toxin_turn_ablation"]["toxin_hits"] < 1
        or rows["toxin_turn_ablation"]["minimum_source_distance"] >= 0.55
    ]
...
def parse_args():
    ...
    parser.add_argument("--world", choices=("all", *WORLD_NAMES), default="all",
                        help="deterministic diagonal hazard fixture; 'all' runs mirrored/distance variants")
...
def main() -> int:
    ...
    output = args.resume or args.output or HERE / "results" / f"embodied_toxin_escape_causal_{timestamp}"
    if args.resume:
        manifest = output / "manifest.json"
        if not manifest.exists():
            raise SystemExit(f"--resume requires an existing manifest: {manifest}")
        prior = json.loads(manifest.read_text())
        if prior.get("experiment") != "embodied_toxin_escape_causal":
        ...
        _write_json(output / "manifest.json", {
            "experiment": "embodied_toxin_escape_causal", "created_utc": datetime.now(timezone.utc).isoformat(),
            ...
            "source_fingerprints": {
                ...
                "embodied_toxin_escape_causal.py": _sha256(Path(__file__).resolve()),
            },
            ...
            "scope": "mirrored and distance-varied diagonal toxin survival through physical toxin currents, PAULA toxin sensors/turn stage, normal CPG/muscles, and MuJoCo; excludes symmetric head-on hazards and long-horizon survival",
    ...
    for world in worlds:
        ...
        for seed in args.seeds:
            ...
            for name in args.cases:
                ...
                print(f"world={world} seed={seed} case={name}: hits={row['toxin_hits']} sensor={row['toxin_sensor_spikes']} turn={row['turn_imbalance_TL_minus_TR']:+d}", flush=True)
    complete = all(
    ...
    print(f"Wrote embodied toxin-escape causal evidence to {output}")
    ...
    if inconclusive:
        ...
        return 1
    print(f"PASS: strict {args.version} diagonal toxin route is causally required across {len(worlds)} deterministic environments")
    return 0
