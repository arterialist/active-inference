...
const stem = path.join(__dirname, '2026-09-09-september8-training-cleanup');
const families = /^20260908_(?:(?:bounded_learning|regional_learning|terminal_family|terminal_shuffled)_(?:paired|swapped)_seed(?:11|23)|content_(?:pairing|swapped)_seed11)$/;
...
if (!process.argv.includes('--apply')) {
  ...
  const manifest = {
    ...
    scope: 'Only intermediate training traces in 18 completed September 8 runs. All September 9 work, all checkpoints and all recall/intervention probes excluded.',
  ...
  for (const dir of new Set(manifest.files.map(f => path.dirname(f.path)))) {
    save(path.join(dir, 'SEPTEMBER8_TRAINING_RAW_PRUNED.json'), {
