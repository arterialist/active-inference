"""Replay the successful body-context contrast through the existing 3D renderer."""
import argparse
import hashlib
import json
...
def run(source, output):
    ...
    movie = output / "body-context-success-replay.mp4"
    if movie.exists():
        ...
        with mujoco.Renderer(hinge.model, height=320, width=640) as renderer:
            for title, prefix, suffix, ticks in stages:
                ...
                for tick in range(ticks):
                    ...
                    draw.text((16, 37), f"{source.name} | saved 3D poses | quarter speed | matched camera | unit omission brake enabled in both", fill=(190, 200, 210), font=small)
                    for column, label in enumerate(("body", "bypass")):
                        ...
                        draw.text((16+640*column, 66), "Body context "+("active" if label == "body" else "bypassed"), fill=color, font=font)
    ...
    (output / "body-context-success-replay.json").write_text(json.dumps(receipt, indent=2)+"\n")
